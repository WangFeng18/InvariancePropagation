...
def adjust_learning_rate(epoch, opt, optimizer, mark):
	"""Sets the learning rate to the initial LR decayed by 0.2 every steep step"""
	if mark == '1':
		baselr = opt.learning_rate1
	if mark == '2':
		baselr = opt.learning_rate2
	steps = np.sum(epoch > np.asarray(opt.lr_decay_epochs))
	if steps > 0:
		new_lr = baselr * (opt.lr_decay_rate ** steps)
...
def parse_option():
	...
	# optimization
	parser.add_argument('--learning_rate1', type=float, default=30)
	parser.add_argument('--learning_rate2', type=float, default=30)
	...
	opt.model_name = '{}_bsz_{}_lr_{}_decay_{}_crop_{}'.format(opt.model_name, opt.batch_size, opt.learning_rate1, opt.weight_decay, opt.crop)
...
def main():
	...
	if args.dataset.startswith('imagenet') or args.dataset.startswith('places'):
		...
		train_loader, val_loader, test_loader = wm811.get_linear_dataloader(args)
	...
	if args.model == 'alexnet':
		...
		model = resnet18_wm811(128, non_linear_head=False, dropout=True)
		model = nn.DataParallel(model)
		classifier = LinearClassifierResNet(args.layer, args.n_label, 'avg', 1, bottleneck=False)
	...
	criterion = LabelSmoothingLoss(args.n_label, args.label_smoothing)

	if not args.adam:
		optimizer1 = torch.optim.SGD(list(model.parameters()),
									lr=args.learning_rate1,
									momentum=args.momentum,
									weight_decay=args.weight_decay)
		optimizer2 = torch.optim.SGD(list(classifier.parameters()),
	...
	for epoch in range(args.start_epoch, args.epochs + 1):
		...
		adjust_learning_rate(epoch, args, optimizer1, '1')
		adjust_learning_rate(epoch, args, optimizer2, '2')
		print("==> training...")

		time1 = time.time()
		train_acc, train_acc5, train_loss = train(epoch, train_loader, model, classifier, criterion, optimizer1, optimizer2, args)
		time2 = time.time()
		logging.info('train epoch {}, total time {:.2f}'.format(epoch, time2 - time1))

		logging.info('Epoch: {}, lr:{} , train_loss: {:.4f}, train_acc: {:.4f}/{:.4f}'.format(epoch, optimizer1.param_groups[0]['lr'], train_loss, train_acc, train_acc5))
		...
		tblogger.log_value('learning_rate', optimizer1.param_groups[0]['lr'], epoch)

		test_acc, test_acc5, test_loss, f1score = validate(val_loader, model, classifier, criterion, args)

		test1_acc, test1_acc5, test1_loss, f1score1 = validate(test_loader, model, classifier, criterion, args)

		if test_acc >= best_acc:
			best_acc = test_acc

		if test1_acc >= test_best_acc:
			test_best_acc = test1_acc
		...
		logging.info(colorful('Epoch: {}, test_loss: {:.4f}, test_acc: {:.4f}/{:.4f}, best_acc: {:.4f}, f1score: {:.4f}'.format(epoch, test1_loss, test1_acc, test1_acc5, test_best_acc, f1score1)))
...
def train(epoch, train_loader, model, classifier, criterion, optimizer1, optimizer2, opt):
	"""
	one epoch training
	"""
...
def validate(val_loader, model, classifier, criterion, opt):
	...
	model.eval()
	classifier.eval()
	f1metric = MultiF1Score(opt.n_label)
	with torch.no_grad():
		end = time.time()
