...
parser.add_argument('--nit', type=int, default=80000,
...
def main():
	...
	if args.model == 'resnet50':
		...
		classifier = nn.Linear(2048, 20).cuda()
	elif args.model == 'resnet18':
		model = resnet18()
		model = nn.DataParallel(model)
		classifier = nn.Linear(512, 20).cuda()
	...
	loader = torch.utils.data.DataLoader(dataset, batch_size=args.batch_size, shuffle=False, num_workers=24, pin_memory=False)
	...
	while it < args.nit:
		...
		count += 1
		if count % 30 == 0:
			evaluate(test_loader, model, classifier, args.eval_random_crops, device)

	print('Evaluation')
	map = evaluate(test_loader, model, classifier, args.eval_random_crops, device)
