...
def main():
	...
	if args.network == 'alexnet_cifar':
		...
		network = ResNet18_cifar(128, dropout=args.dropout)
	...
	# create criterion
	criterion = objective.PointLoss(args.t)
	...
	try:
		for i_epoch in range(start_epoch, args.max_epoch):
			...
			train(i_epoch, network, criterion, optimizer, train_loader, device, memory_bank)
...
def train(i_epoch, network, criterion, optimizer, dataloader, device, memory_bank):
	...
	for data in pbar:
		...
		L = criterion(output, index, memory_bank)
		losses.add(L.item())
...
#python instance.py --gpus '0' --max_epoch 200 --exp '/data/HierachicalAggregation_exp/cifar10/' --res_path '/data/HierachicalAggregation_exp/cifar10/record.csv' --dataset 'cifar10' --n_workers 4 --t 0.1 --network 'resnet18_cifar'