...
class PreActBottleneck(nn.Module):
	...
	def __init__(self, in_planes, planes, stride=1, enable_shortcut=True):
...
class PreActResNet(nn.Module):
	def __init__(self, block, num_blocks, num_classes=128, input_channels=3):
		...
		self.bn = batch_norm(512*block.expansion)
	...
	def forward_convs(self, x):
		out = self.conv1(x)
		conv1 = self.bn(self.relu(out))
		out = self.padding(out)
		out = self.maxpool(out)

		out = self.layer1(out)
		conv2 = self.bn(self.relu(out))

		out = self.layer2(out)
		conv3 = self.bn(self.relu(out))

		out = self.layer3(out)
		conv4 = self.bn(self.relu(out))
